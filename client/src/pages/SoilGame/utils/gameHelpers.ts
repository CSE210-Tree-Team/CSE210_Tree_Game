import {
  ...
  export function hasUncollectedResource(map: Node[][], pos: Position): boolean {
    ...
    return node !== null && node.resource !== null && !node.collected;
  }
  ...
  /** Remove an element from inventory (returns null if not enough) */
  ...
  /** Submit an element to a quest. Returns updated quest or null if element not needed. */
  ...
  export function formatLocationInfo(pos: Position, map: Node[][]): string[] {
    const node = getNodeAt(map, pos);
    const resource = node?.resource && !node.collected ? node.resource : 'None';
    const moves = getPossibleMoves(pos, map.length);
    const resourceSymbol = resource !== 'None' ? ` [${getElementSymbol(resource as ElementType)}]` : '';
